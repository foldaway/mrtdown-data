...
export async function lineSummaryQuery(componentId: string, days: number) {
  ...
  const sql = `
    WITH bounds AS (
      SELECT
        (NOW() AT TIME ZONE 'Asia/Singapore') - INTERVAL '${days} days' AS start_time,
        (NOW() AT TIME ZONE 'Asia/Singapore') AS end_time
    ),
    ...
    intervals_expanded_all AS (
      ...
      JOIN LATERAL (
        SELECT gs_local
        FROM generate_series(
          DATE_TRUNC('day', (iv.start_at AT TIME ZONE 'Asia/Singapore')::DATE),
          DATE_TRUNC('day', (COALESCE(iv.end_at, b.end_time) AT TIME ZONE 'Asia/Singapore')::DATE),
          INTERVAL 1 day
        ) AS gs(gs_local)
      ) gs ON TRUE
