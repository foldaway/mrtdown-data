...
export async function operatorIssueCountsCumulativeQuery(
  operatorId: string,
  ...
  const sql = `
    ...
    current_intervals AS (
      SELECT
        i.id AS issue_id,
        i.type,
        GREATEST(iv.start_at AT TIME ZONE 'Asia/Singapore', bo.current_start_time) AS start_clipped,
        LEAST(
          COALESCE(iv.end_at AT TIME ZONE 'Asia/Singapore', bo.current_end_time, NOW() AT TIME ZONE 'Asia/Singapore'),
          bo.current_end_time
        ) AS end_clipped
      FROM issues i
      ...
      WHERE (iv.start_at AT TIME ZONE 'Asia/Singapore') < bo.current_end_time
        AND COALESCE(iv.end_at AT TIME ZONE 'Asia/Singapore', bo.current_end_time, NOW() AT TIME ZONE 'Asia/Singapore') > bo.current_start_time
    ),
    prior_intervals AS (
      SELECT
        i.id AS issue_id,
        i.type,
        GREATEST(iv.start_at AT TIME ZONE 'Asia/Singapore', bo.prior_start_time) AS start_clipped,
        LEAST(
          COALESCE(iv.end_at AT TIME ZONE 'Asia/Singapore', bo.prior_end_time, NOW() AT TIME ZONE 'Asia/Singapore'),
          bo.prior_end_time
        ) AS end_clipped
      FROM issues i
      JOIN issue_intervals iv ON i.id = iv.issue_id
      ...
      JOIN operator_lines ol ON ol.line_id = il.line_id
      CROSS JOIN bounds bo
      WHERE (iv.start_at AT TIME ZONE 'Asia/Singapore') < bo.prior_end_time
        AND COALESCE(iv.end_at AT TIME ZONE 'Asia/Singapore', bo.prior_end_time, NOW() AT TIME ZONE 'Asia/Singapore') > bo.prior_start_time
    ),
    current_agg AS (
      SELECT
        'current' AS period,
