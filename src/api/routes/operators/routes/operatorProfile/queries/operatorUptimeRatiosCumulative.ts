...
export async function operatorUptimeRatiosCumulativeQuery(
  operatorId: string,
  ...
  return await withConnection(async (connection) => {
    ...
    intervals_expanded AS (
      ...
      JOIN operator_lines ol ON ol.line_id = ic.line_id
      ...
      WHERE i.type IN ('disruption', 'maintenance')
        AND (iv.start_at AT TIME ZONE 'Asia/Singapore') < p.end_time
        AND COALESCE(iv.end_at AT TIME ZONE 'Asia/Singapore', p.end_time) > p.start_time
        AND (iv.start_at AT TIME ZONE 'Asia/Singapore') < sw.service_end
        AND COALESCE(iv.end_at AT TIME ZONE 'Asia/Singapore', p.end_time) > sw.service_start
    ),
    downtime_totals AS (
      SELECT
        period_name,
