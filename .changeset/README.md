...
Use `npm run changeset` in feature PRs that change package behavior. The release
workflow turns merged changesets on `main` into a version PR, then publishes
non-private packages after that version PR is merged.
